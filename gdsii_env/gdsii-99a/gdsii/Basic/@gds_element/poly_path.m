...
% Convert paths with multiple path segments; Ulf Griesmann, August 2012

% check if input is a path
if ~strcmp(pelm.etype, 'path')
   error('gds_element.poly_path :  input must be path element.');
end

% copy element and remove path specific properties
belm  = pelm;
belm.etype = 'boundary';
belm.data.nume = pelm.data.nume;   % number of elements does not change
rmfield(belm.data, 'ptype');       % remove ptype field
rmfield(belm.data, 'width');       % remove width
rmfield(belm.data, 'ext');         % data for path type 4

% convert paths
if iscell(pelm.data.xy)
   belm.data.xy = {};
   for k=1:length(pelm.data.xy)
      belm.data.xy{k} = ...
         path_to_boundary(pelm.data.xy{k}, pelm.data.ptype, pelm.data.width, pelm.data.ext);
   end
else
   belm.data.xy = path_to_boundary(pelm.data.xy, pelm.data.ptype, pelm.data.width, pelm.data.ext);
end
