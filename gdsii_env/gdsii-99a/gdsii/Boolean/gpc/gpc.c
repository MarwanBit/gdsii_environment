...
NOTE: This software was modified for use with Ocatave / MATLAB. Functions
for reading / writing of polygons have been removed.
Ulf Griesmann, August 2012
...
#define MALLOC(p, b, s, t) {if ((b) > 0) { \
                            p= (t*)mxMalloc(b); if (!(p)) { \
                            mexPrintf("gpc mxMalloc failure: %s\n", s); \
                            exit(0);}} else p= NULL;}

#define FREE(p)            {if (p) {mxFree(p); (p)= NULL;}}
